...
export class CompaniesComponent implements OnInit {
  ...
  save(data: any) {
    if (this.formdata.invalid) {
      ...
      if (data.id == 0) {
        this.api.post("api/companies", data).subscribe((result: any) => {
          this.api.showSuccess("Record added successfully.");
          this.bind();
        });
      } else {
        this.api.put("api/companies/" + data.id, data).subscribe((result: any) => {
  ...
  edit(id: number) {
    this.api.get("api/companies/" + id).subscribe((result: any) => {
  ...
  delete(id: number) {
    Notiflix.Confirm.show(
      ...
      'No',
      () => {
        this.api.delete("api/companies/" + id).subscribe((result: any) => {
